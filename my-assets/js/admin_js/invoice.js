...
function addInputField_invoice(t) {
    ...
    if (count == limits)
        ...
        e.innerHTML = "<td><input type='text' name='product_name' onkeypress='invoice_productList(" + count +
            ");' class='form-control productSelection common_product' placeholder='Nama Produk' id='" + a +
...
function addInputField_target(divName) {
    ...
    if (count == limits) alert("You have reached the limit of adding " + count + " inputs");
        ...
        var a = "product_name_" + count;
        var tabindex = count * 6;
        tab1 = tabindex + 1;
        newdiv = document.createElement("tr");

        newdiv.innerHTML = "<td><input type='text' name='product_name' onkeypress='invoice_productList(" + count +
            ");' class='form-control productSelection common_product' placeholder='Search Product' id='" + a +
            "' required tabindex='" + tab1 + "'><input type='hidden' class='common_product autocomplete_hidden_value  product_id_" + count +
            "' name='product_id[]' id='SchoolHiddenId'/></td><td><input type='text' name='target[]' class='form-control' value='1'></td><td> <select name='cmbCode[]' id='cmbCode_" + count + "' class='form-control'  required></select></td><td><button class='btn btn-danger red' type='button'  onclick='deleteRowdbtvoucher(this)'><i class='fa fa-close'></i></button></td>";
...
function addInputField_invoice_dynamic(t) {
    ...
    if (count == limits)
        ...
        e.innerHTML = "<td><input type='text' name='product_name' onkeypress='invoice_productList(" + count +
            ");' class='form-control productSelection common_product' placeholder='Nama Produk' id='" + a +
            "' required tabindex='" + tab1 + "'><input type='hidden' class='common_product autocomplete_hidden_value  product_id_" + count +
            "' name='product_id[]' id='SchoolHiddenId'/></td><input type='hidden' name='desc[]'' class='form-control text-right ' tabindex='" +
            tab2 + "'/><td><select class='form-control basic-single'  onchange='invoice_product_batch(" + count + ")' id='serial_no_" + count + "' name='serial_no[]' required aria-hidden='true' tabindex='" + tab3 +
            "'><option></option></select></td> <td><input type='text' name='available_quantity[]' id='' class='form-control text-right common_avail_qnt available_quantity_" +
            count + "' value='0' readonly='readonly' /></td><td><input class='form-control text-right common_name unit_" + count +
            " valid' value='None' readonly='' aria-invalid='false' type='text'></td><td> <input type='text' name='product_quantity[]' value='1' required='required' onkeyup='japasys_invoice_quantity_calculate(" +
            count + ");' onchange='japasys_invoice_quantity_calculate(" + count + ");' id='total_qntt_" + count + "' class='common_qnt total_qntt_" +
            ...
            "' /><input type='hidden' value='' name='discount_type' id='discount_type_" + count + "'></td><input type='hidden' name='discountvalue[]'  id='discount_value_" + count +
            "' class='form-control text-right common_discount' placeholder='0' min='0' tabindex='" + tab13 + "' readonly /><td><input type='text' name='discount2[]' onkeyup='japasys_invoice_quantity_calculate("
            + count + ");' onchange='japasys_invoice_quantity_calculate(" + count + ");' id='discount2_" + count + "' class='form-control text-right common_discount' placeholder='0.0' min='0' tabindex='" + tab5 +
            "' /><input type='hidden' value='' name='discount_type' id='discount_type_" + count + "'></td><input type='hidden' name='discountvalue2[]'  id='discount_value2_" + count +
            "' class='form-control text-right common_discount' placeholder='0' min='0' tabindex='" + tab14 + "' readonly /><td><input type='text' name='discount3[]' onkeyup='japasys_invoice_quantity_calculate("
            + count + ");' onchange='japasys_invoice_quantity_calculate(" + count + ");' id='discount3_" + count + "' class='form-control text-right common_discount' placeholder='0.0' min='0' tabindex='" + tab5 +
            "' /><input type='hidden' value='' name='discount_type' id='discount_type_" + count + "'></td><input type='hidden' name='discountvalue3[]'  id='discount_value3_" + count +
            "' class='form-control text-right common_discount' placeholder='0' min='0' tabindex='" + tab15 + "' readonly /><td class='text-right'><input class='common_total_price total_price form-control text-right' type='text' name='total_price[]' id='total_price_" +
            count + "' value='0' readonly='readonly'/></td><td>" + tbfild + "<input type='hidden' id='all_discount_" + count
            + "' class='total_discount dppr' name='discount_amount[]'/><input type='hidden' id='all_discount2_" + count
            + "' class='total_discount dppr' name='discount_amount[]'/><input type='hidden' id='all_discount3_" + count
            + "' class='total_discount dppr' name='discount_amount[]'/><button tabindex='" + tab5 +
            "' style='text-align: right;' class='btn btn-danger' type='button' value='Delete' onclick='deleteRow_invoice(this)'><i class='fa fa-close'></i></button></td>",
...
//Quantity calculat
"use strict";
function japasys_invoice_quantity_calculate(item) {
    var quantity = $("#total_qntt_" + item).val();
    var available_quantity = $(".available_quantity_" + item).val();
    var price_item = $("#price_item_" + item).val();
    var invoice_discount = $("#invoice_discount").val();
    var discount = $("#discount_" + item).val();
    var discount2 = $("#discount2_" + item).val();
    var discount3 = $("#discount3_" + item).val();
    ...
    if (available_quantity != 0) {
        if (parseInt(quantity) > parseInt(available_quantity)) {
            ...
            for (var i = 0; i < taxnumber; i++) {
                ...
                japasys_invoice_quantity_calculate(item);
            }
        }
    }
    ...
    if (quantity > 0 || discount > 0 || vat_percent > 0) {
        if (dis_type == 1) {
            ...
            if (discount2 > 0) {
                var price2 = temp;
                var dis2 = +(temp * discount2 / 100);
                $("#discount_value2_" + item).val(dis2.toFixed(0));
                $("#all_discount2_" + item).val(dis2.toFixed(0));
                //Total price calculate per product
                var temp2 = price2 - dis2;
                // product wise vat start
                var vat = +(temp2 * vat_percent / 100);
                $("#vat_value_" + item).val(vat);
                $("#total_price_" + item).val(temp2.toFixed(0));
                for (var i = 0; i < taxnumber; i++) {
                    var tax = (temp2) * $("#total_tax" + i + "_" + item).val();
                    ttletax += Number(tax);
                    $("#all_tax" + i + "_" + item).val(tax);
                }
                if (discount3 > 0) {
                    var price3 = temp2;
                    var dis3 = +(temp2 * discount3 / 100);
                    $("#discount_value3_" + item).val(dis3.toFixed(0));
                    $("#all_discount3_" + item).val(dis3.toFixed(0));
                    //Total price calculate per product
                    var temp3 = price3 - dis3;
                    // product wise vat start
                    var vat = +(temp3 * vat_percent / 100);
                    $("#vat_value_" + item).val(vat);
                    $("#total_price_" + item).val(temp3.toFixed(0));
                    for (var i = 0; i < taxnumber; i++) {
                        var tax = (temp3) * $("#total_tax" + i + "_" + item).val();
                        ttletax += Number(tax);
                        $("#all_tax" + i + "_" + item).val(tax);
                    }
                } else {
    ...
    if (invoice_edit_page == 1) {
        ...
        var url = base_url + "invoice/invoice/japasys_showpaymentmodal";
        $.ajax({
            type: "post",
            url: url,
...
function deleteRow_invoice(t) {
    ...
    if (1 == a)
        ...
        $("#normalinvoice > tbody > tr td input.common_qnt").each(function () {
            ...
            $(this).attr('onkeyup', 'japasys_invoice_quantity_calculate(' + common_qnt + ');');
            $(this).attr('onchange', 'japasys_invoice_quantity_calculate(' + common_qnt + ');');
        });
        var common_rate = 1;
...
window.onload = function () {
    $('body').addClass("sidebar-mini sidebar");
}
...
function customer_autocomplete(sl) {
    ...
    var options = {
        ...
        source: function (request, response) {
            ...
            $.ajax({
                url: base_url + "invoice/invoice/japasys_customer_autocomplete",
                method: 'post',
                dataType: "json",
                data: {
                    term: request.term,
...
function invoice_productList(sl) {
    ...
    var options = {
        ...
        source: function (request, response) {
            ...
            $.ajax({
                url: base_url + "invoice/invoice/japasys_autocomplete_product",
                method: 'post',
                dataType: "json",
                data: {
                    term: request.term,
...
function invoice_product_batch(sl) {
    ...
    $.ajax({
        url: base_url + "invoice/invoice/japasys_batchwise_productprice",
        method: 'post',
        dataType: "json",
        data: {
            prod_id: prod_id,
