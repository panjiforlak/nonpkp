...
class Account_model extends CI_Model
{
  ...
  function dfs($HeadName, $HeadCode, $oResult, $visit, $d, $account_name_indo)
  {
    ...
    else if ($d == 1) echo "<li class=\"jstree-open\"><a href='javascript:' onclick=\"loadCoaData('" . $HeadCode . "')\" class=\"form-control jstreelip\">$account_name_indo <span class=\"bal_span\"> $balance</span><span class=\"bal_span_pre\">$opening</span></a>";
    else echo "<li class=\"jstreeli\"><a href='javascript:' class=\"form-control\" onclick=\"loadCoaData('" . $HeadCode . "')\"> $account_name_indo <span class=\"bal_span\"> $balance</span> <span class=\"bal_span_pre\">$opening</span></a>";
    $p = 0;
    for ($i = 0; $i < count($oResult); $i++) {
      ...
      if (!$visit[$i]) {
        if ($HeadName == $oResult[$i]->PHeadName) {
          ...
          if ($p == 0) echo "<ul>";
          $p++;
          $this->dfs($oResult[$i]->HeadName, $oResult[$i]->HeadCode, $oResult, $visit, $d + 1, $oResult[$i]->account_name_indo ?: $oResult[$i]->HeadName);
        }
      }
    }
    if ($p == 0)
  ...
  public function supplier_payment_insert()
  {
    ...
    if ($multipaytype[0] != 0) {
      foreach ($multipaytype  as $multipaytype) {
        ...
        $paymethod = array(
          'VNo'            =>  $voucher_no,
          'Vtype'          =>  "Pembayaran Pembelian",
          'VDate'          =>  $VDate,
          'COAID'          =>  $multipaytype,
          'Narration'      =>  'Pelunasan ke ' . $supinfo->supplier_name,
          'Debit'          =>  0,
    ...
    if ($multipaytype[0] != 0) {
      foreach ($multipaytype  as $multipaytype) {
        ...
        $paymethod = array(
          ...
          'VDate'          =>  $VDate,
          'COAID'          =>  $multipaytype,
          'Narration'      =>  'Penerimaan Pembayaran dari  ' . $customerinfo->customer_name,
  ...
  public function listCash() // perubahan : penambahan function listCash
  {
    return  $data = $this->db->select("*")
      ->from('acc_coa')
      ->where('PHeadName', 'Cash in Banks')
      ->order_by('HeadName')
      ->get()
      ->result();
  }
  ...
  public function insert_debitvoucher()
  {
    ...
    for ($i = 0; $i < count($dAID); $i++) {
      ...
      $cinsert = array(
        ...
        'VDate'          =>  $VDate,
        'COAID'          =>  $cAID,
        'Narration'      =>  'Pembayaran dari ' . $headinfo->HeadName, //perubahan :sebelumnya "Debit voucher from"
        'Debit'          =>  0,
        'Credit'         =>  $Damnt,
        'IsPosted'       => $IsPosted,
        'CreateBy'       => $CreateBy,
  ...
  public function insert_creditvoucher()
  {
    ...
    for ($i = 0; $i < count($cAID); $i++) {
      ...
      $cinsert = array(
        ...
        'VDate'          =>  $VDate,
        'COAID'          =>  $dAID,
        'Narration'      =>  'Penerimaan ke ' . $headinfo->HeadName,
        'Debit'          =>  $Cramnt,
        'Credit'         =>  0,
        'IsPosted'       => $IsPosted,
        'CreateBy'       => $CreateBy,
  ...
  public function update_creditvoucher()
  {
    ...
    for ($i = 0; $i < count($cAID); $i++) {
      ...
      $cinsert = array(
        ...
        'VDate'          =>  $VDate,
        'COAID'          =>  $dAID,
        'Narration'      =>  'Penerimaan ke ' . $headinfo->account_name_indo ?: $headinfo->HeadName,
        'Debit'          =>  $Cramnt,
        'Credit'         =>  0,
        'IsPosted'       => $IsPosted,
        'CreateBy'       => $CreateBy,
  ...
  {
    $sql = "SELECT * FROM acc_coa WHERE acc_coa.IsTransaction=1 AND acc_coa.IsGL=1 AND acc_coa.HeadType='A' AND acc_coa.IsActive=1 AND acc_coa.HeadCode LIKE '111%'";
  ...
  public function trial_balance_report($FromDate, $ToDate, $WithOpening)
  {
    ...
    $sql = "SELECT * FROM acc_coa WHERE IsGL=1 AND IsActive=1 AND HeadType IN ('I','E','O') ORDER BY HeadCode";
  ...
  public function equity_info_tax($head_name)
  {
  ...
  public function equity_balance($head_code, $from_date, $to_date)
  {
    ...
    $this->db->from('acc_transaction');
    $this->db->like('COAID', $head_code); // perubahan : sebelumnya where jadi like
    $this->db->where('VDate >=', $from_date);
    $this->db->where('VDate <=', $to_date);
    $this->db->where('IsAppove', 1);
    return  $records = $this->db->get()->result_array();
  ...
  public function income_balance($head_code, $from_date, $to_date)
  {
    ...
    $this->db->from('acc_transaction');
    $this->db->like('COAID', $head_code); // perubahan : dari where jadi like
    $this->db->where('VDate >=', $from_date);
    $this->db->where('VDate <=', $to_date);
    $this->db->where('IsAppove', 1);
    return  $records = $this->db->get()->result_array();
  ...
  public function pmethod_dropdown()
  {
    $data = $this->db->select('*')
      ->from('acc_coa')
      ->where_in('PHeadName', ['Cash In Boxes', 'Cash in Banks']) //perubahan : sebelumnya pakai where(pheadname,cash in boxes)
      ->get()
      ->result();
