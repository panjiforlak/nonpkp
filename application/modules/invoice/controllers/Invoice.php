...
class Invoice extends MX_Controller
{
    ...
    function japasys_invoice_form()
    {
        ...
        $data['module']        = "invoice";
        $data['salesman']      = $this->invoice_model->get_sales();
    ...
    public function japasys_invoice_list()
    {
    ...
    // new module
    function japasys_target_form()
    {

        $data['get_periode']     = $this->invoice_model->get_period();
        $data['title']         = "Target Sales - Period";
        $data['module']        = "invoice";
        $data['page']          = "add_target_form";

        echo modules::run('template/layout', $data);
    }
    function japasys_target_sales_form($param = null)
    {
    ...
    function japasys_target_insert()
    {
        ...
        $targetInsert      = $this->invoice_model->add_target();
        // echo modules::run('template/layout');
    }
    function japasys_target_period_insert()
    {

        $this->invoice_model->add_target_period();
        $data['module']     = "invoice";
    ...
    public function japasys_invoice_details($invoice_id = null)
    {
        $bank       = $this->invoice_model->get_bank();
        $invoice_detail     = $this->invoice_model->retrieve_invoice_html_data($invoice_id);
        $salesman   = $this->invoice_model->get_sales($invoice_detail[0]['sales_by']);
        $salesadmin   = $this->invoice_model->get_salesadmin($invoice_detail[0]['admin_by']);
        ...
        if (!empty($invoice_detail)) {
            foreach ($invoice_detail as $k => $v) {
                ...
                $invoice_detail[$k]['due_date'] = $invoice_detail[$k]['due_date'];
        ...
        $data = array(
            ...
            'sales_by'          => $salesman[0]['first_name'] . ' ' . $salesman[0]['last_name'],
            'admin_by'          => $salesadmin[0]['first_name'] . ' ' . $salesadmin[0]['last_name'],
            'bank'              => $bank,
            'email_address'     => $invoice_detail[0]['email_address'],
            'contact'           => $invoice_detail[0]['contact'],
            'invoice_details'   => $invoice_detail[0]['invoice_details'],
            'total_amount'      => number_format($invoice_detail[0]['total_amount'], 0, '.', ','),
            'subTotal_quantity' => $subTotal_quantity,
            'total_discount'    => number_format($invoice_detail[0]['total_discount'], 0, '.', ','),
            'total_discount_cal' => $invoice_detail[0]['total_discount'],
            'total_vat'         => number_format($invoice_detail[0]['total_vat_amnt'], 0, '.', ','),
            'total_tax'         => number_format($invoice_detail[0]['total_tax'], 0, '.', ','), //perubahan sebelumnya total_tax valuenya
    ...
    public function japasys_invoice_pad_print($invoice_id)
    {
        $invoice_detail = $this->invoice_model->retrieve_invoice_html_data($invoice_id);
        $taxfield = $this->db->select('*')
            ->from('tax_settings')
        ...
        $data = array(
            ...
            'customer_email'   => $invoice_detail[0]['customer_email'],
            'final_date'       => $invoice_detail[0]['final_date'],
            'print_setting'    => $this->invoice_model->japasys_print_settingdata(),
            'invoice_details'  => $invoice_detail[0]['invoice_details'],
    ...
    public function japasys_invoice_pos_print($invoice_id = null)
    {
        $invoice_detail = $this->invoice_model->retrieve_invoice_html_data($invoice_id);
        $taxfield = $this->db->select('*')
            ->from('tax_settings')
    ...
    public function japasys_download_invoice($invoice_id = null)
    {
        $invoice_detail = $this->invoice_model->retrieve_invoice_html_data($invoice_id);
        $taxfield = $this->db->select('*')
            ->from('tax_settings')
    ...
    public function japasys_manual_sales_insert()
    {
        ...
        if ($this->form_validation->run() === true) {
            ...
            if (!empty($invoice_id)) {
                ...
                if ($normal == 1) {
                    $printdata       = $this->invoice_model->japasys_invoice_pos_print_direct($invoice_id);
    ...
    public function japasys_showpaymentmodal()
    {
    ...
    public function japasys_edit_invoice($invoice_id = null)
    {
    ...
    public function japasys_update_invoice()
    {
        $this->form_validation->set_rules('customer_id', display('customer_name'), 'required|max_length[15]');
        $this->form_validation->set_rules('invoice_no', display('invoice_no'), 'required|max_length[25]');
        $this->form_validation->set_rules('multipaytype[]', display('payment_type'), 'required');
        $this->form_validation->set_rules('product_id[]', display('product'), 'required|max_length[20]');
        $this->form_validation->set_rules('product_quantity[]', display('quantity'), 'required|max_length[20]');
        $this->form_validation->set_rules('product_rate[]', display('rate'), 'required|max_length[20]');
    ...
    public function japasys_gui_pos()
    {
        $taxfield = $this->db->select('tax_name,default_value')
            ->from('tax_settings')
            ->get()
    ...
    public function japasys_customer_autocomplete()
    {
    ...
    public function japasys_batchwise_productprice()
    {
    ...
    public function japasys_invoice_details_directprint($invoice_id = null)
    {
        $invoice_detail     = $this->invoice_model->retrieve_invoice_html_data($invoice_id);
        $taxfield = $this->db->select('*')
            ->from('tax_settings')
    ...
    public function japasys_terms_form($id = null)
    {
    ...
    public function japasys_terms($id = null)
    {
