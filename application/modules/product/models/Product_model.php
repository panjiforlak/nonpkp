...
class Product_model extends CI_Model
{
    ...
    public function getProductList($postData = null)
    {
        ...
        foreach ($records as $record) {
            ...
            if ($this->permission1->method('manage_product', 'delete')->access()) {
                ...
                $button .= '<a href="' . $base_url . 'product/product/japasys_deleteproduct/' . $record->product_id . '" class="btn btn-xs btn-danger "  onclick="' . $jsaction . '"><i class="fa fa-trash"></i></a>';
            }
    ...
    public function japasys_barcode_productdata($id)
    {
        return $this->db->select('*')
            ->from('product_information')
            ->where('product_id', $id)
